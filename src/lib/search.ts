...
const pages = [
  ...
  ['Audio thoughts', '/audio-thoughts'], ['Pages', '/documents'], ['Progress', '/progress'], ['Timeline', '/timeline'],
];
...
export function searchEntries(data: Workspace): SearchEntry[] {
  return [
    ...
    ...(data.runningNotes ?? []).map(e => ({ title: e.title, href: `/audio-thoughts?id=${encodeURIComponent(e.id)}`, kind: 'Audio thought', body: `${e.tags.join(' ')} ${e.body}` })),
