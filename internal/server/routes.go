...
func (s *Server) RegisterRoutes() http.Handler {
	r := chi.NewRouter()
	metricConfig := otelchimetric.NewBaseConfig(s.config.OTelServiceName, otelchimetric.WithAttributesFunc(httpAttributes))
	r.Use(
		captureStatus,
	...
	r.Group(func(r chi.Router) {
		...
		r.Post("/api/running/clips", s.uploadRunningClip)
		r.Get("/api/running/clips/{noteId}/{clipId}/audio", s.runningAudio)
		r.Post("/api/running/clips/{noteId}/{clipId}/retranscribe", s.retranscribe)
		r.Get("/api/running/{noteId}/status", s.runningStatus)
		r.Get("/api/connections", s.readConnections)
		r.Post("/api/connections/import", s.importConnections)
		r.Get("/api/connections/photo", s.connectionPhoto)
		r.Post("/api/connections/photo", s.uploadConnectionPhoto)
		r.Delete("/api/connections/photo", s.deleteConnectionPhoto)
