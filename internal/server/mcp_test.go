...
func (h *oauthHarness) consent(clientID, redirect, challenge string) string {
	...
	page := w.Body.String()
	if !strings.Contains(page, "Test &lt;client&gt;") || w.Header().Get("X-Frame-Options") != "DENY" {
...
func TestOAuthDiscoveryAndAuthorize(t *testing.T) {
	...
	// Missing PKCE redirects an error to the verified callback.
	...
	if u, _ := url.Parse(w.Header().Get("Location")); w.Code != http.StatusFound || u.Query().Get("error") != "invalid_request" || u.Query().Get("state") != "xyz" {
		t.Fatal(w.Code, w.Header().Get("Location"))
	}
...
func TestMCPOverOAuth(t *testing.T) {
	...
	if read["text"] == "" || read["nextOffset"] != float64(10) || read["revision"] == "" {
		t.Fatal(read)
	}
