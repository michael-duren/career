...
def canonical(sha):
    ...
    token, _, _ = request_json(f'https://ghcr.io/token?service=ghcr.io&scope=repository:{REPO}:pull',
...
def payload(tag, sha, digest):
    validate(tag, sha)
    if not re.fullmatch(r'sha256:[0-9a-f]{64}', digest):
        raise ValueError('Expected immutable SHA256 image digest')
    return {'ref': 'main', 'inputs': {'app': 'career', 'tag': tag, 'sha': sha,
            'images': json.dumps({'career': f'{IMAGE}@{digest}'}, separators=(',', ':'))}}
