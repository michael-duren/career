...
- `POST /api/goals/steps/move` with `{ stepId, from, fromRevision, to, toRevision, index }` → `{ goals, revisions }` for every changed goal. Reorders a mini goal (goal step) within its goal or moves it to another goal in one transaction; `index` is its position in the destination afterwards (clamped to the end). Both revisions must match.
